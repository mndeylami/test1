...
export default () => {
	step('Test: Start', async browser => {
		...
		await browser.takeScreenshot()
		await browser.click(By.xpath('//span/ohp-icon-font/span'))
		await browser.switchTo().frame(window.frames(1))
		await browser.sendKeys("Ctrl+Tab")
		await browser.wait(Until.elementIsVisible(By.visibleText('Outlook')))
		//await browser.wait(Until.elementIsVisible(By.xpath('//div[2]/div/button/span[2]')))
		await browser.takeScreenshot()
		await browser.click(By.xpath('//div[2]/div/button/span[2]'))
		//await browser.takeScreenshot()
	})
}
